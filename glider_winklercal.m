...
%% Plot profiles aligned with calibration cast

%%%% Issues to resolve: 1) Why is the shape of the Winklers different from
%%%% that of the glider measurements for O2 saturation? Need to apply a
%%%% pressure correction? 2) Why are Winkler values higher than glider for
%%%% O2 saturation but lower for concentration? Why not same gain
%%%% correction for both?
...
subplot(223)
    hold on;
d = find(G363.daten < endtime & G363.daten > starttime);
    plot(G363.oxygen_saturation(d), G363.depth_interp(d), '.', 'color', nicecolor('ry'))
d = find(G453.daten < endtime & G453.daten > starttime);
    plot(G453.oxygen_saturation(d), G453.depth_interp(d), 'b.')
...
set(gca,'YDir','reverse'); 
axis([80 120 1 1005])
legend('Glider 363, raw','Glider 453, raw','BCP O_2 from cast','Dave O_2 from cast','location','southeast')
...
set(gca,'YDir','reverse'); 
axis([80 120 1 1005])
legend('Glider 363','Glider 453','BCP O_2 from cast','location','southeast')
